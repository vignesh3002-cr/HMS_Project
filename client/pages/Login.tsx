...
export default function Login() {
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log("ROLE TYPE:", response?.user?.role_type);
      ...
      const roleType = String(
        response.user.role_type ?? ""
      )
        .trim()
        .toUpperCase();

      console.log("Detected role_type:", roleType);

      // --------------------------------------------------
      // 4. CLEAR OLD LOGIN/PERMISSION STATE
      // --------------------------------------------------
      /*
       * This is important when an admin logs out and a doctor
       * logs in using the same browser.
       *
       * It prevents old admin permission/navigation state from
       * being reused for the doctor.
       */
      try {
        ...
        console.warn(
          "Could not clear old permission state:",
          storageError
        );
      }

      // --------------------------------------------------
      // 5. SAVE NEW AUTHENTICATION INFORMATION
      // --------------------------------------------------
      ...
      // --------------------------------------------------
      // 6. LOAD PERMISSIONS FOR CURRENT USER
      // --------------------------------------------------
      try {
        await refetchPermissions();

        console.log(
          "Permissions loaded for:",
          roleType
        );
      } catch (permissionError) {
        console.warn(
          "Permission loading failed after login:",
          permissionError
        );

        /*
         * Do not stop login if permission loading fails.
         * PermissionContext can retry later.
         */
      }
          ...
          <div className="relative z-10 flex flex-col justify-center gap-5 p-8 h-full">
            ...
            <div className="flex flex-col gap-4 p-6 rounded-lg border border-white/10 bg-white/[0.16] backdrop-blur-xl">
              ...
              <div className="w-fit p-[10px] rounded-[4px] bg-white/20 backdrop-blur-md">
                ...
                >
                  <path
                    d="M4 14H6V9H4V14ZM12 14H14V4H12V14ZM8 14H10V11H8V14ZM8 9H10V7H8V9ZM2 18C1.45 18 0.979167 17.8042 0.5875 17.4125C0.195833 17.0208 0 16.55 0 16V2C0 1.45 0.195833 0.979167 0.979167 0.5875C0.979167 0.195833 1.45 0 2 0H16C16.55 0 17.0208 0.195833 17.4125 17.4125C17.8042 0.979167 17.8042 16V16C16 18.55 15.55 18 15.55 18H2Z"
                    fill="white"
                  />
                </svg>
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm font-medium text-clinical-blue-light leading-[22.75px]">
                  High-fidelity data visualization and editorial clarity for
            ...
            <div className="flex flex-col gap-4 p-6 rounded-lg border border-white/10 bg-white/[0.16] backdrop-blur-xl">
              ...
              <div className="w-fit p-[10px] rounded-[4px] bg-white/20 backdrop-blur-md">
                ...
                >
                  <path
                    d="M2 20C1.45 20 0.979167 19.8042 0.5875 19.4125C0.195833 19.0208 0 18.55 0 18V6C0 5.45 0.195833 4.97917 0.5875 4.5875C0.979167 4.19583 1.45 4 2 4H6V2C6 1.45 6.19583 0.979167 6.979167 0.195833 7.45 0 8 0H12C12.55 0 13.0208 0.195833 13.4125 0.5875C13.8042 0.979167 14 1.45 14 2V4H18C18.55 4 19.0208 4.195833 19.4125 4.5875C19.8042 4.97917 20 5.45 20 6V18C20 18.55 19.8042 19.0208 19.4125 19.4125C19.0208 19.8042 18.55 20 18H2ZM2 18H18V6H2V18ZM8 4H12V2H8V4ZM9 13V16H11V13H14V11H11V8H9V11H6V13H9Z"
                    fill="white"
                  />
                </svg>
              </div>
