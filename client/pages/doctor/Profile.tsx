...
function formatDoctorFullName(e: DashboardEmployee | null): string {
  if (!e) return "Doctor";
      ...
      {branches.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {branches.map((b) => (
            <span
              key={b.branch_id}
              className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg"
            >
...
export default function DoctorDetailView() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (!employeeId) {
          throw new Error("No employee ID linked to the logged-in user.");
        }

        const doctorResponse = await doctorDashboardApi.getDoctor(employeeId);
        const doctorData = doctorResponse.data.data;

        setDoctorDetail(doctorData as EmployeeDetailResponse);
        setLoading(false);
      } catch (error: any) {
        console.error("[DoctorDetailView] Failed to load doctor profile:", error);
        setError(
    ...
    return (
      <div className="min-h-screen bg-[#f7f9fc] flex items-center justify-center">
        <div className="text-center">
          <p className="text-[#5f6672]">{error}</p>
          <button
  ...
  if (!doctorDetail) {
    return (
      <div className="min-h-screen bg-[#f7f9fc] flex items-center justify-center">
        <p className="text-[#5f6672]">No profile data found.</p>
      </div>
    );
  }
  ...
  const doctorSpecialization =
    doctorDetail.doctorProfile?.specialization ||
    doctorEmployee?.specialization ||
    "—";
  const doctorQualification =
    doctorDetail.doctorProfile?.qualification || doctorEmployee?.qualification || "—";
  const activeBranches = (doctorDetail.branches ?? []).filter(
    (b) => b.status === 1
  );
  ...
  const doctorIsAvailable =
    doctorEmployee?.emp_status === true || doctorDetail.user?.user_status === 0;
  const doctorPhoto = doctorEmployee?.employee_photo_URL || "";
  const doctorLicenseNo =
    doctorDetail.doctorProfile?.license_no || doctorEmployee?.license_no || "—";
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap mb-3">
                ...
                <span
                  className={doctorIsAvailable ? "text-[#0b955e]" : "text-[#9aa1ab]"}
                  style={{ fontSize: "13px" }}
                >
