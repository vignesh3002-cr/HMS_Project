...
// import { APPOINTMENTS, AVAILABILITY, FEEDBACK, TREND_DATA } from "../data/mockData";
// import type { Appointment, AvailabilitySlot, FeedbackEntry, TrendPoint } from "../types";
// 
// /* ---------- TopBar ---------- */
// 
...
// }
// 
// function TopBar({ doctorName, doctorId, rating, reviewCount, date }: TopBarProps) {
//   return (
//     <div className="flex items-start justify-between">
//       <div>
//         <h2 className="text-2xl font-bold text-gray-900">Welcome back, {doctorName}</h2>
...
// }
// 
// function StatCard({ label, value, iconBg, icon, footer }: StatCardProps) {
//   return (
...
// }
// 
// function AppointmentsTable({ appointments, visibleCount = 4, showAll, onToggleShowAll }: AppointmentsTableProps) {
//   const visibleAppointments = showAll ? appointments : appointments.slice(0, visibleCount);
...
//                   <img src={appt.avatarUrl} alt={appt.patientName} className="w-8 h-8 rounded-full object-cover" />
//                   <span className="font-medium text-gray-800">{appt.patientName}</span>
//                 </div>
//               </td>
//               <td className="py-3 text-gray-500">{appt.dateTime}</td>
//               <td className="py-3 text-gray-500">{appt.phone}</td>
//               <td className="py-3">
//                 <span className={`text-xs font-medium px-3 py-1 rounded-full ${STATUS_STYLES[appt.status]}`}>
//                   {appt.status}
//                 </span>
//               </td>
//             </tr>
...
// const RADIUS = 6;
// 
// // Draws a bar with rounded top corners only (flat bottom), so it sits on the baseline correctly.
...
// }
// 
// function AppointmentTrendsChart({ data }: { data: TrendPoint[] }) {
//   const max = Math.max(...data.map((d) => Math.max(d.completed, d.rescheduled)));
//   const scale = (v: number) => (v / max) * CHART_HEIGHT;
//   const chartWidth = data.length * (BAR_WIDTH * 2 + BAR_GAP + GROUP_GAP);
// 
//   return (
...
//           <p className="text-sm text-gray-400 mt-0.5">Weekly performance analysis</p>
//         </div>
// 
//         <div className="flex items-center gap-6">
//           <span className="flex items-center gap-2 text-sm text-gray-600">
//             <span className="w-3 h-3 rounded-full bg-blue-900" /> Completed
//           </span>
//           <span className="flex items-center gap-2 text-sm text-gray-600">
//             <span className="w-3 h-3 rounded-full bg-gray-300" /> Rescheduled
//           </span>
// 
...
//             <g key={d.label}>
//               <path
//                 d={roundedTopBarPath(groupX, CHART_HEIGHT - completedH, BAR_WIDTH, completedH, RADIUS)}
//                 fill="#1E3A8A"
//               />
//               <path
//                 d={roundedTopBarPath(
...
// /* ---------- AvailabilityPanel ---------- */
// 
// function AvailabilityPanel({ location, slots }: { location: string; slots: AvailabilitySlot[] }) {
//   return (
//     <div className="bg-white rounded-2xl border border-gray-100 p-5">
//       <div className="flex items-center justify-between mb-4 gap-3">
//         <h3 className="font-semibold text-gray-900 shrink-0">Availability</h3>
...
// }
// 
// function PatientFeedback({ entries }: { entries: FeedbackEntry[] }) {
//   return (
//     <div className="bg-white rounded-2xl border border-gray-100 p-5">
//       <h3 className="font-semibold text-gray-900 mb-4">Patient Feedback</h3>
// 
//       <div className="space-y-4">
//         {entries.map((entry) => (
//           <div key={entry.id} className="pb-4 border-b border-gray-50 last:border-0 last:pb-0">
//             <div className="flex items-center justify-between mb-1">
//               <span className="text-sm font-semibold text-gray-800">{entry.name}</span>
//               <Stars rating={entry.rating} />
//             </div>
//             <p className="text-xs text-gray-500 leading-relaxed">&ldquo;{entry.comment}&rdquo;</p>
//           </div>
//         ))}
//       </div>
// 
//       <button className="text-sm font-medium text-blue-700 hover:underline mt-2">Read All Reviews</button>
//     </div>
//   );
// }
// 
...
//       <TopBar doctorName="Dr. Jenkins" doctorId="DOC-99283" rating={4.9} reviewCount={128} date="May 30, 2026" />
// 
...
//               footer={<span className="text-xs text-gray-400">12 slots remaining today</span>}
//             />
//           </div>
// 
//           <AppointmentsTable
...
//             onToggleShowAll={() => setShowAllAppointments((prev) => !prev)}
//           />
//           <AppointmentTrendsChart data={TREND_DATA} />
//         </div>
// 
//         <div className="space-y-6">
//           <AvailabilityPanel location="Central Hospital (Tambaram)" slots={AVAILABILITY} />
